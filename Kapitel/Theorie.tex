...
\subsubsection{Formulierung des Problems nach Lagrange}
\label{sub.sub.Lagrange}
Mit den in Abschnitt~\ref{sub.sub.Annahmen} getroffenen Annahmen erhalten wir durch den Ansatz von Lagrange'schen Tensoren die nachfolgenden Formeln \cite{Cazzolato.2011}.

Die potentielle und die kinetische Energie von Arm~1:

\begin{equation*}
E_{p1} = 0 \qquad E_{k1}=\frac{1}{2}(v^T_{1c}m_1v_{1c}+\omega^T_1J_1\omega_1)=\frac{1}{2}\dot{\theta}^2_1(m_1l_1+J_{1zz})
\end{equation*}
Die potentielle und kinetische Energie von Arm~2:
\begin{equation*}
